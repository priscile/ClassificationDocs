...
# Note: this was modified by Luke Vilnis for UMass CS585 on 11/21/2013 
# to use numpy instead of defunct Numeric/MLab packages
...
def rosen_der(x):
    ...
    xm_m1 = x[:-2]
    xm_p1 = x[2:]
    der = MLab.zeros(x.shape,x.dtype)
...
def fmin(func, x0, args=(), xtol=1e-4, ftol=1e-4, maxiter=None, maxfun=None, fulloutput=0, printmessg=1):
    ...
    while (funcalls < maxfun and iterations < maxiter):
        if (max_1(Num.ravel(abs(sim[1:]-sim[0]))) <= xtol \
            and max_1(abs_1(fsim[0]-fsim[1:])) <= ftol):
            break
...
def line_search_BFGS(f, xk, pk, gfk, args=(), c1=1e-4, alpha0=1):
    ...
    if (phi_a0 <= phi0 + c1*alpha0*derphi0.all()):
        return alpha0, fc, 0
    ...
    if (phi_a1 <= phi0 + c1*alpha1*derphi0.any()):
        return alpha1, fc, 0
...
def fminNCG(f, x0, fprime, fhess_p=None, fhess=None, args=(), avextol=1e-5, maxiter=None, fulloutput=0, printmessg=1):
    ...
    xk = x0
    k = 0
    while (Num.add.reduce(abs_1(update)) > xtol) and (k < maxiter):
